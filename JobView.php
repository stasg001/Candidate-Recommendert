...
<body>
    ...
    <div class="content">
        ...
        <div class='sbtn'><button type="button" class="btn btn-info btn-xs" onclick="jobadd()">Job Add</button></div><br/>  
        <?php
            $href="JobRecommend.php?id=";
            $namearray = array();
            $descarray = array();
            $idarray = array();
            if($idarray=mysqli_query( $conn, "SELECT Id FROM Job") or die("Could not execute query: " .mysqli_error($conn)))
            if($namearray=mysqli_query( $conn, "SELECT Name FROM Job") or die("Could not execute query: " .mysqli_error($conn)))
            if($descarray=mysqli_query( $conn, "SELECT Description FROM Job") or die("Could not execute query: " .mysqli_error($conn)))
            while ($row=mysqli_fetch_row($namearray) and $row1=mysqli_fetch_row($descarray) and $id=mysqli_fetch_row($idarray)){
                for($b = 0; $b <sizeof($row); $b = $b + 1){
                     echo'<a style="text-decoration:none" href="'.$href.$id[$b].'"><div class="card">
