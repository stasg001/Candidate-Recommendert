...
<body>
    ...
    <div class="section-3">
        <div class="w-row">
            ...
            <div class="w-col w-col-5">
                <div class="text-block"><a style="text-decoration:none" href="LandingPage.php">T A A P</a></div>
            </div>
    ...
    <div class="content">
        ...
        <?php
            ...
            $selectedid = array();
            $selectarray = array();
            if($interviewarray=mysqli_query( $conn, "SELECT CandidateId FROM Interview where EmployeeId='$id'") or die("Could not execute query: " .mysqli_error($conn))) 
            if($jobarray=mysqli_query( $conn, "SELECT JobDesc FROM Interview where EmployeeId='$id'") or die("Could not execute query: " .mysqli_error($conn)))
            while($res = mysqli_fetch_row($interviewarray) and $row=mysqli_fetch_row($jobarray)){
                for($b = 0; $b < sizeof($res); $b++){
                    ...
                    <p>'.$row[$b].'</p>';
                    $flag = 0;
                    if($selectedid = mysqli_query( $conn, "SELECT CandId FROM SelectedCandidates") or die("Could not execute query: " .mysqli_error($conn)))
                    while($cows = mysqli_fetch_row($selectedid)) {
                        for($c = 0; $c < sizeof($cows); $c = $c+ 1){
                            $selectarray[$c] = $cows[$c];
                            if ($res[$b] == $selectarray[$c]){
                                $flag = 1;
                                break;
                            }              
                        }
                    ...
                    if ($flag == 1){
                        echo '<button type="button" class="btn btn-success">Selected</button>';
                    }
                    else{
